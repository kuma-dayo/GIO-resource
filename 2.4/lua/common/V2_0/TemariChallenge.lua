...
function Initialize()
	...
	table.insert(variables,{ name = "has_hidden", value = 0,no_refresh = true})
    --记录uid的低四位
    table.insert(variables,{ name = "challenger_uid1", value = 0})
    --记录uid的高位，challenger_uid2*10000+challenger_uid就是最后正确的uid
    table.insert(variables,{ name = "challenger_uid2", value = 0})

    --记录放球的小数位
    table.insert(variables,{ name = "pos_x1", value = 0})
    table.insert(variables,{ name = "pos_z1", value = 0})
end
