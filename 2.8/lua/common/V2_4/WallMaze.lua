...
function Initialize()
    ...
    for i = 1,#StartWallMap do
        table.insert(variables,{ config_id=51000000+i,name = "wall_"..i, value = 0})
    end
    for i = 1,#StartBlockerMap do
        table.insert(variables,{ config_id=52000000+i,name = "blocker_"..i, value = 0})
    end
    
    if (SpecialBlockerMap ~= nil) then
        for i = 1,#SpecialBlockerMap do
            for j = 1,#SpecialBlockerMap[i] do
                table.insert(variables,{ config_id=53000000+1000*i+j,name = "special_blocker_"..i.."_"..j, value = 0})
            end
        end
    end
    table.insert(variables,{ config_id=50000001,name = "is_moving", value = 0, no_refresh = true})
    
    table.insert(variables,{ config_id=50000002,name = "has_succeeded", value = 0, no_refresh = true})
end
