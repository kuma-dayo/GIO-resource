...
function Initialize()
    ...
    --燃料数
    table.insert(variables,{ name = "fuel_num", value = 0})
    --零件数
    table.insert(variables,{ name = "part_num", value = 0})
    --金币数
    table.insert(variables,{ name = "coin_num", value = 0})

    
    --各个分块是否存活，存成一个四位二进制数方便管理
    table.insert(variables,{ name = "raft_part_alive", value = 1111})


    --发动机的能量数，相当于发动机的血量
    table.insert(variables,{ name = "engine_energy", value = 0})
    --船是否处于运动状态
    table.insert(variables,{ name = "is_running", value = 0})


    --临时能量池，缓存SLC上来的能量恢复值，每秒与运行时的能量消耗统一结算，防止0能量时反复充能放能导致振荡
    table.insert(variables,{ name = "engine_energy_pool", value = 0})


    --记录当前是否因没有能量处于停船状态，如果是，则在下一次能量大于0时恢复一次船的行动
    table.insert(variables,{ name = "has_stopped_by_no_energy", value = 0})
    --记录当前是否因路障导致停船，如果是，下次路障死掉的时候要恢复一次行动
    table.insert(variables,{ name = "has_stopped_by_road_block", value = 0})

    --停船操作计数。当计数小于等于0时，才能启动船
    table.insert(variables,{ name = "stop_raft_op_num", value = 0})

    --当前阶段怪物是否已经全死了
    table.insert(variables,{ name = "current_stage_monster_all_killed", value = 0})
    --当前阶段的障碍物是否被消灭
    table.insert(variables,{ name = "current_stage_obstacled_all_killed", value = 0})
    
    --检查点计数，用于给客户端同步的
    table.insert(variables,{ name = "checkpoint_index", value = 1})


    --燃料保底机制触发次数，超过guarantee_times后不再触发
    table.insert(variables,{ name = "fuel_guarantee_times", value = 0})


    --分块低血量提示间隔时间
    table.insert(variables,{ name = "can_show_low_hp_reminder", value = 1})
