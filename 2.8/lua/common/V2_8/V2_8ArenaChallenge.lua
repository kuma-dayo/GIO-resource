...
function Initialize()
    ...
    for k,v in pairs(counting_challenge) do
        if v.challenge_id ~= nil then
            ...
            table.insert(variables,{ name = v.var_name, value = 0})
        end
    end
    --阶段数
    table.insert(variables,{ name = "current_stage", value = 0})
    --小怪潮的index
    ...
    table.insert(variables,{ name = "tide_op_num", value = 0})
    --转阶段条件完成计数
    table.insert(variables,{ name = "state_change_condition_num", value = 0})

    
    --是否是第一组精英怪
    table.insert(variables,{ name = "is_first_elite", value = 1})

    --阶段1开始时间
    table.insert(variables,{ name = "stage1_start_time", value = 0})
end
