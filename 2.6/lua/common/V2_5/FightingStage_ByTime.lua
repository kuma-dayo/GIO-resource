...
function Initialize()
    ...
    table.insert(variables,{ name = "is_noswitch_punishment", value = 0})
    --用于记录当前精英是否在场，精英在场时，怪物潮不做刷新
    table.insert(variables,{ name = "is_elite_on_ground", value = 0})
    --用于记录当前怪物潮是否结束，当精英死亡时判断当前怪物潮是否处于结束状态，如果是，则直接加载下一波怪物潮
    table.insert(variables,{ name = "is_current_minion_tide_end", value = 0})
    --用于记录是否已经创建空气墙，防止玩家卡在空气墙外
    table.insert(variables,{ name = "is_air_wall_created", value = 0})
    --用于记录场上的怪物总数，当需要切怪物潮时，需要先判断场上已经没有怪物了
    table.insert(variables,{ name = "monster_count", value = 0})
    --用于标记需要监听场上怪物死亡直到怪物数量为0
    table.insert(variables,{ name = "last_end_tide", value = 0})
    table.insert(variables,{ name = "refresh_end_tide", value = 0})
end
