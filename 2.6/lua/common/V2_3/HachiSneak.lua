...
function Initialize_Group(triggers, suites)
	...
	--进度保存
	table.insert(variables,{ name = "saved_progress", value = 0, no_refresh =true})
	--挑战状态标记
	table.insert(variables,{ name = "challenge_state", value = 0})
	--路点状态标记
	table.insert(variables,{ name = "waypoint_set", value = 0})
	--最多4只动物 存活记录
	table.insert(variables,{ name = "animal", value = 10000})

	--第一个营地的挑战是否已通过任务正常开启（如果不是的话，接到第二次任务通知，要再开一次挑战）
	table.insert(variables,{ name = "quest_done", value = 0, no_refresh =true})
