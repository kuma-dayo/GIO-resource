...
function LF_Initialize_Group(triggers, suites)
	...
	table.insert(variables,{ name = "gallery_state", value = 0, no_refresh = true})
	--父挑战状态 0-初始 1-进了地城但未开始 2-进行中（正在激活古代符文 3-进行中（启动遗迹控制台）4-进行中（最终挑战） 5-全部完成
	table.insert(variables,{ name = "father_state", value = 0, no_refresh = true})

	--钥匙房 激活古代符文进度
	table.insert(variables,{ name = "key_progress", value = 0, no_refresh = true})

	--子挑战用的触发器
	table.insert(variables,{ name = "catchKey", value = 0, no_refresh = false})

	--地脉异常等级
	table.insert(variables,{ name = "floor_level", value = 1, no_refresh = true})
