...
function LF_Init_Monster_Group()
	...
	table.insert(variables, { name = "tide_ptr", value = 0}) -- TIDE
	table.insert(variables, { name = "challenge_group", value = 0}) -- 主控group
	table.insert(variables, { name = "left_monsters", value = 0}) -- 该wave的怪物总数
	table.insert(variables, { name = "monster_tide_index", value = 1}) -- 真正的mini刷怪波次
	table.insert(variables, { name = "monster_kill_count", value = 0}) -- 死亡（包含逃走）的怪物
	-- table.insert(variables, { name = "planNum", value = 1, no_refresh = true})
	table.insert(variables, { name = "monster_wave_ptr", value = 1}) -- WAVE

	table.insert(variables, { name = "currentTideMonsters", value = 0}) -- 当前tide的怪物数量
