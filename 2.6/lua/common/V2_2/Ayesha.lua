...
local temp_Variables = {
	{ name = "NowOrderKey", value = 1, no_refresh = true },	--用于记录当前是哪个点：order中的key 用于断线重连时恢复进度
	{ name = "NowState", value = 0, no_refresh = true }, --用于记录当前点是哪个状态：0待探测 1待交互 用于断线重连时恢复进度
	{ name = "LD_Finish", value = 0, no_refresh = false }, --修改该值将触发LD_Finish
	{ name = "AddChallengeProgress", value = 0, no_refresh = false }, --修改该值将触发challenge进度+1
	{ name = "IS_IN_CHALLENGE", value = 0, no_refresh = false }, --防止重复开启挑战
}
