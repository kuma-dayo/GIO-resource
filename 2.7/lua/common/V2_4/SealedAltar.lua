...
function Initialize()
    ...
    --需要外部领取奖励后，将锁打开，开启玩法
    table.insert(variables,{ config_id=50000001,name = "play_lock", value = 0, no_refresh = true})

    --solution_state:记录当前各个解法的状态
    --0 = 未开放、1 = 已开放、2 = 已完成、3 = 已领取
    --solution_progress记录当前各个解法的完成进度，取值为迄今为止正确的位数
    for i = 1,#SolutionList do
        table.insert(variables,{ config_id=51000000+i,name = "solution_state_"..i, value = 0, no_refresh = true})
        table.insert(variables,{ config_id=52000000+i,name = "solution_progress_"..i, value = 0, no_refresh = true})
    end
end
