...
function action_EVENT_GADGET_CREATE_3486002(context, evt)
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 432203486, 3486001, {57}) then
...
function condition_EVENT_SELECT_OPTION_3486003(context, evt)
	-- 判断是gadgetid 3486001 option_id 57
	if 3486001 ~= evt.param1 then
		return false	
	end
	
	if 57 ~= evt.param2 then
		return false
	end
...
function action_EVENT_SELECT_OPTION_3486003(context, evt)
	-- 删除指定group： 432203486 ；指定config：3486001；物件身上指定option：75；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 432203486, 3486001, 75) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : del_work_options_by_group_configId")
		return -1
	end
	
		-- 卸载指定gadget
		if 0 ~= ScriptLib.RemoveEntityByConfigId(context, 432203486, EntityType.GADGET, 3486001 ) then
