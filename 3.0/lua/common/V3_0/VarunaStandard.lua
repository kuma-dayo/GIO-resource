...
function LF_DelFishingPoint(context, weatherState)
    ...
    if suite ~= 0 then
        -- ScriptLib.KillEntityByConfigId(context, { group_id = base_info.group_id, config_id = pointId})
        ...
        ScriptLib.PrintContextLog(context, "## [VarunaStandard] del all fish points")
    end
end
