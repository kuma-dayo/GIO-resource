...
local BossMarana_Trigger = {
    ...
    --{ keyWord = "DelayRevertTeleport", event = EventType.EVENT_TIME_AXIS_PASS, source = "DelayRevertTeleport", trigger_count = 0},
    -- 配合CutScene的时间轴
    { keyWord = "BossAppearMovie", event = EventType.EVENT_TIME_AXIS_PASS, source = "BossAppearMovie", trigger_count = 0},
    { keyWord = "LastBattle", event = EventType.EVENT_TIME_AXIS_PASS, source = "LastBattle", trigger_count = 0},
    ...
    -- CutScene结束时
    --[[
    { keyWord = "BossAppearMovie", event = EventType.EVENT_CUTSCENE_END, source = tostring(BattleCfg.BossAppearCutScene.id), trigger_count = 0},
    { keyWord = "LastBattle", event = EventType.EVENT_CUTSCENE_END, source = tostring(BattleCfg.RanaroCrownCutScene.id), trigger_count = 0},
    { keyWord = "BossDieEnd", event = EventType.EVENT_CUTSCENE_END, source = tostring(BattleCfg.BossDieCutScene.id), trigger_count = 0},
    { keyWord = "BossBattleEnd", event = EventType.EVENT_CUTSCENE_END, source = tostring(BattleCfg.SalanAppearCutScene.id), trigger_count = 0},
    --]]
...
function action_MissionRevert(context,evt)
    ...
    if bossBattleProgress > 0 then
        LF_InitLevel(context)
        ScriptLib.PrintContextLog(context, "## TD_BossMarana MissionRevert 触发回滚")
    end
    return 0
end
...
function action_FirstNearMarana(context,evt)
    ...
    -- 移除黑泥
    ScriptLib.RemoveEntityByConfigId(context, base_info.group_id, EntityType.GADGET, BattleCfg.BlackMud )
...
--[[
-- 6/19删除:修改兰那罗移动赶路的时机
-- 离开Boss，兰纳罗仓鼠球开始移动     ["RanaroStartMove"] = {1,6,12},
function action_RanaroStartMove(context,evt)

    local regionConfigID = evt.param1
    if regionConfigID ~= BattleCfg.RanaroStartMoveRegion then
        return 0
    end

    if LF_CheckProgressNoMatchGroup(context,ProgressGroup.RanaroStartMove,"action_RanaroStartMove") then
        return 0
    end


    local waveNum = LF_GetWaveNum(context,ProgressGroup.RanaroStartMove,"action_RanaroStartMove")
    LF_ProgressPush(context,"action_RanaroStartMove")

    -- 根据波次，设置兰纳罗的移动路线
    ScriptLib.SetGroupVariableValue(context,"RanaroPointIndex",waveNum)
    -- 将当前设为第一步，之后兰纳罗每次被贴近会移动到下一个点
    ScriptLib.SetGroupVariableValue(context,"RanaroPointStep",1)
...
function action_BossRecover(context,evt)
    ...
    if waveNum >= 3 then
        ...
        ScriptLib.InitTimeAxis(context, "LastBattle",{BattleCfg.RanaroCrownCutScene.time + 0.5}, false)
