...
triggers = {
	...
	-- 消亡态 杀宝箱
	{ config_id = 1102006, name = "GROUP_LOAD_102006", event = EventType.EVENT_GROUP_LOAD, source = "", condition = "condition_EVENT_GROUP_LOAD_102006", action = "action_EVENT_GROUP_LOAD_102006", trigger_count = 0 }
}
...
suites = {
	{
		...
		gadgets = { 102002 },
		regions = { },
		triggers = { "GADGET_STATE_CHANGE_102003", "GROUP_LOAD_102004", "GADGET_STATE_CHANGE_102005", "GROUP_LOAD_102006" },
		rand_weight = 100
	}
}
...
-- 触发条件
function condition_EVENT_GROUP_LOAD_102004(context, evt)
	-- 检测指定三态物件的grassState是否为对应状态，其中1为正常态、2为激化态、3为消亡态
	if 102002 ~= evt.param2 then
		return false
	end
...
-- 触发操作
function action_EVENT_GROUP_LOAD_102004(context, evt)
	-- 创建id为102001的gadget
	if 0 ~= ScriptLib.CreateGadget(context, { config_id = 102001 }) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : create_gadget")
	  return -1
...
function action_EVENT_GADGET_STATE_CHANGE_102005(context, evt)
		...
		if 0 ~= ScriptLib.KillEntityByConfigId(context, { config_id = 102001 }) then
...
-- 触发条件
function condition_EVENT_GROUP_LOAD_102006(context, evt)
	-- 检测指定三态物件的grassState是否为对应状态，其中1为正常态、2为激化态、3为消亡态
	if 102002 ~= evt.param2 then
		return false
	end
