...
suites = {
	{
		...
		regions = { 31011 },
		triggers = { "VARIABLE_CHANGE_31004", "QUEST_START_31013", "GADGET_STATE_CHANGE_31016", "GADGET_STATE_CHANGE_31022" },
		rand_weight = 100
	}
}
...
-- 触发条件
function condition_EVENT_GADGET_STATE_CHANGE_31016(context, evt)
	if GadgetState.GearStart ~= ScriptLib.GetGadgetStateByConfigId(context, 133304031, 31001) then
		return false
	end
	
	if GadgetState.GearStart ~= ScriptLib.GetGadgetStateByConfigId(context, 133304031, 31002) then
		return false
	end
...
-- 触发操作
function action_EVENT_GADGET_STATE_CHANGE_31016(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 133304079, suite = 2 }) then
