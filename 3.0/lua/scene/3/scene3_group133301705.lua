...
-- 装置
gadgets = {
	{ config_id = 705005, gadget_id = 70900050, pos = { x = -533.726, y = -38.576, z = 3814.896 }, rot = { x = 353.242, y = 353.916, z = 20.215 }, level = 33, state = GadgetState.GearStop, persistent = true, area_id = 25 }
}
...
function action_EVENT_ENTER_REGION_705001(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 133301705, suite = 2 }) then
...
function action_EVENT_ANY_MONSTER_DIE_705006(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 133301705, suite = 3 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
