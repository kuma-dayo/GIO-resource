...
suites = {
	...
	},
	{
		...
		regions = { 130019 },
		triggers = { "GROUP_LOAD_130007", "ENTER_REGION_130019" },
		rand_weight = 100
	},
	{
		-- suite_id = 3,
...
-- 触发条件
function condition_EVENT_ANY_GADGET_DIE_130006(context, evt)
	if 130014 ~= evt.param1 then
		return false
	end
...
-- 触发操作
function action_EVENT_ANY_GADGET_DIE_130006(context, evt)
...
function action_EVENT_GROUP_LOAD_130007(context, evt)
	...
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 133302065, 65012, GadgetState.GearStart) then
		...
		-- 卸载指定gadget
		if 0 ~= ScriptLib.RemoveEntityByConfigId(context, 133302130, EntityType.GADGET, 130011 ) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : remove_gadget_by_configid")
			return -1
		end
	
		-- 卸载指定gadget
		if 0 ~= ScriptLib.RemoveEntityByConfigId(context, 133302130, EntityType.GADGET, 130012 ) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : remove_gadget_by_configid")
			return -1
		end
	
		-- 卸载指定gadget
		if 0 ~= ScriptLib.RemoveEntityByConfigId(context, 133302130, EntityType.GADGET, 130017 ) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : remove_gadget_by_configid")
			return -1
		end
...
function action_EVENT_GADGET_STATE_CHANGE_130010(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 133302130, suite = 2 }) then
