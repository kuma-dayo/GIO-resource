...
triggers = {
	...
	-- 延迟卸载不必要的东西
	{ config_id = 1076021, name = "GROUP_LOAD_76021", event = EventType.EVENT_GROUP_LOAD, source = "", condition = "condition_EVENT_GROUP_LOAD_76021", action = "" },
	-- 保底
...
suites = {
	{
		...
		regions = { },
		triggers = { "GADGET_STATE_CHANGE_76004" },
		rand_weight = 100
	},
	{
		-- suite_id = 2,
		...
		regions = { },
		triggers = { "GADGET_STATE_CHANGE_76013", "GADGET_STATE_CHANGE_76014", "GADGET_STATE_CHANGE_76015", "GROUP_LOAD_76022" },
		rand_weight = 100
	},
	{
		-- suite_id = 3,
	...
	},
	{
		...
		-- description = ,
		monsters = { },
		gadgets = { 76007, 76008, 76009, 76010, 76018, 76019 },
		regions = { },
		triggers = { },
		rand_weight = 100
	}
...
function action_EVENT_GADGET_STATE_CHANGE_76004(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 133302076, suite = 2 }) then
...
function action_EVENT_GADGET_STATE_CHANGE_76013(context, evt)
	...
	-- 将configid为 76019 的物件更改为状态 GadgetState.GearStart
	if 0 ~= ScriptLib.SetGadgetStateByConfigId(context, 76019, GadgetState.GearStart) then
	...
	-- 将configid为 76009 的物件更改为状态 GadgetState.GearStart
	if 0 ~= ScriptLib.SetGadgetStateByConfigId(context, 76009, GadgetState.GearStart) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_configId")
			return -1
		end 
	
	-- 将configid为 76007 的物件更改为状态 GadgetState.GearStart
	if 0 ~= ScriptLib.SetGadgetStateByConfigId(context, 76007, GadgetState.GearStart) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_configId")
			return -1
		end 
	
	-- 将configid为 76008 的物件更改为状态 GadgetState.GearStart
	if 0 ~= ScriptLib.SetGadgetStateByConfigId(context, 76008, GadgetState.GearStart) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_configId")
			return -1
		end 
	
	-- 将configid为 76018 的物件更改为状态 GadgetState.GearStart
	if 0 ~= ScriptLib.SetGadgetStateByConfigId(context, 76018, GadgetState.GearStart) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_configId")
			return -1
		end 
...
-- 触发条件
function condition_EVENT_GROUP_LOAD_76021(context, evt)
	-- 判断变量"123"为1
