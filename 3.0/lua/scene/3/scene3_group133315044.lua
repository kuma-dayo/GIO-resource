...
gadgets = {
	...
	[44036] = { config_id = 44036, gadget_id = 70310192, pos = { x = 558.654, y = 142.925, z = 2423.077 }, rot = { x = 0.000, y = 325.805, z = 0.000 }, level = 1, area_id = 20 },
...
suites = {
	{
		...
		-- description = ,
		monsters = { },
		gadgets = { 44005, 44006, 44007, 44008, 44009, 44010, 44011, 44012, 44013, 44014, 44015, 44016, 44020, 44021, 44022, 44029, 44030, 44031, 44032, 44034, 44035, 44036, 44037, 44038, 44039, 44040, 44041, 44042, 44043, 44052, 44054, 44055, 44061, 44062 },
		regions = { },
...
function action_EVENT_ANY_MONSTER_DIE_44049(context, evt)
	...
	-- 将configid为 44052 的物件更改为状态 GadgetState.Default
	if 0 ~= ScriptLib.SetGadgetStateByConfigId(context, 44052, GadgetState.Default) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_configId")
			return -1
		end 
	
	-- 将configid为 44054 的物件更改为状态 GadgetState.Default
	if 0 ~= ScriptLib.SetGadgetStateByConfigId(context, 44054, GadgetState.Default) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_configId")
			return -1
		end 
	
	-- 将configid为 44055 的物件更改为状态 GadgetState.Default
	if 0 ~= ScriptLib.SetGadgetStateByConfigId(context, 44055, GadgetState.Default) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_configId")
			return -1
		end 
...
function action_EVENT_MONSTER_BATTLE_44060(context, evt)
	-- 创建标识为"fight"，时间节点为{2,14,26}的时间轴，true用于控制该时间轴是否循环
	ScriptLib.InitTimeAxis(context, "fight", {2,14,26}, true)
