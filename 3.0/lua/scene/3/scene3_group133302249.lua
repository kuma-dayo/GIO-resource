...
-- 怪物
monsters = {
	[249002] = { config_id = 249002, monster_id = 24030101, pos = { x = -660.726, y = 215.873, z = 3005.940 }, rot = { x = 0.000, y = 359.778, z = 0.000 }, level = 33, drop_id = 1000101, affix = { 6116, 6117 }, isElite = true, pose_id = 101, area_id = 24 }
}
...
gadgets = {
	...
	[249005] = { config_id = 249005, gadget_id = 70310193, pos = { x = -669.981, y = 216.843, z = 3008.116 }, rot = { x = 341.009, y = 359.721, z = 355.865 }, level = 33, persistent = true, area_id = 24 },
	...
	[249010] = { config_id = 249010, gadget_id = 70310191, pos = { x = -665.911, y = 215.291, z = 2991.614 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 27, state = GadgetState.GearStop, persistent = true, area_id = 24 },
...
-- 区域
regions = {
	[249011] = { config_id = 249011, shape = RegionShape.SPHERE, radius = 7, pos = { x = -666.151, y = 215.297, z = 2991.732 }, area_id = 24 }
}
...
suites = {
	{
		...
		regions = { 249011 },
		triggers = { "ENTER_REGION_249011", "TIME_AXIS_PASS_249012", "TIME_AXIS_PASS_249017", "ANY_GADGET_DIE_249027", "GADGET_STATE_CHANGE_249028", "GADGET_STATE_CHANGE_249029", "GADGET_STATE_CHANGE_249030", "LUA_NOTIFY_249032" },
		rand_weight = 100
	},
	{
		-- suite_id = 2,
...
-- 触发操作
function action_EVENT_ENTER_REGION_249011(context, evt)
	-- 将configid为 249010 的物件更改为状态 GadgetState.Default
	if 0 ~= ScriptLib.SetGadgetStateByConfigId(context, 249010, GadgetState.Default) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_configId")
			return -1
		end 
	
	-- 创建标识为"123"，时间节点为{15,20}的时间轴，true用于控制该时间轴是否循环
	ScriptLib.InitTimeAxis(context, "123", {15,20}, true)
...
function condition_EVENT_TIME_AXIS_PASS_249012(context, evt)
	if "123" ~= evt.source_name or 1 ~= evt.param1 then
		return false
	end
...
function action_EVENT_TIME_AXIS_PASS_249012(context, evt)
	-- 将configid为 249010 的物件更改为状态 GadgetState.GearStop
	if 0 ~= ScriptLib.SetGadgetStateByConfigId(context, 249010, GadgetState.GearStop) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_configId")
			return -1
		end 
...
-- 触发条件
function condition_EVENT_TIME_AXIS_PASS_249017(context, evt)
	if "123" ~= evt.source_name or 2 ~= evt.param1 then
		return false
	end
...
-- 触发操作
function action_EVENT_TIME_AXIS_PASS_249017(context, evt)
	-- 将configid为 249010 的物件更改为状态 GadgetState.Default
	if 0 ~= ScriptLib.SetGadgetStateByConfigId(context, 249010, GadgetState.Default) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_configId")
			return -1
