...
-- 装置
gadgets = {
	{ config_id = 489001, gadget_id = 70330238, pos = { x = -279.278, y = 196.543, z = 3278.295 }, rot = { x = 347.512, y = 319.357, z = 352.027 }, level = 27, area_id = 22 },
	{ config_id = 489002, gadget_id = 70290591, pos = { x = -276.407, y = 199.445, z = 3266.188 }, rot = { x = 19.075, y = 192.832, z = 180.000 }, level = 27, vision_level = VisionLevelType.VISION_LEVEL_LITTLE_REMOTE, area_id = 22 }
}
...
suites = {
	{
		...
		regions = { },
		triggers = { "GADGET_STATE_CHANGE_489003", "GROUP_LOAD_489004", "GADGET_STATE_CHANGE_489005" },
		rand_weight = 100
	},
	{
		-- suite_id = 2,
...
-- 触发条件
function condition_EVENT_GADGET_STATE_CHANGE_489003(context, evt)
	if 489001 ~= evt.param2 or GadgetState.GearStop ~= evt.param1 then
		return false
	end
...
-- 触发操作
function action_EVENT_GADGET_STATE_CHANGE_489003(context, evt)
		-- 卸载指定gadget
		if 0 ~= ScriptLib.RemoveEntityByConfigId(context, 133301489, EntityType.GADGET, 489002 ) then
...
-- 触发条件
function condition_EVENT_GROUP_LOAD_489004(context, evt)
	if GadgetState.GearStop ~= ScriptLib.GetGadgetStateByConfigId(context, 133301489, 489001) then
		return false
	end
...
-- 触发操作
function action_EVENT_GROUP_LOAD_489004(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 133301489, suite = 2 }) then
