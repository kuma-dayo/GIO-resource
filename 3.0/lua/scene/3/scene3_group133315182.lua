...
suites = {
	{
		-- suite_id = 1,
		-- description = ,
		monsters = { 182007, 182008 },
		gadgets = { 182001, 182003, 182004, 182006, 182009, 182010, 182011, 182012 },
		regions = { },
		triggers = { "GADGET_STATE_CHANGE_182005", "GADGET_STATE_CHANGE_182013", "GROUP_LOAD_182014", "GROUP_LOAD_182015" },
		rand_weight = 100
	},
	{
		-- suite_id = 2,
...
-- 触发条件
function condition_EVENT_GROUP_LOAD_182015(context, evt)
	-- 检测指定三态物件的grassState是否为对应状态，其中1为正常态、2为激化态、3为消亡态
	if 182001 ~= evt.param2 then
		return false
	end
...
-- 触发操作
function action_EVENT_GROUP_LOAD_182015(context, evt)
	-- 创建id为182002的gadget
	if 0 ~= ScriptLib.CreateGadget(context, { config_id = 182002 }) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : create_gadget")
	  return -1
