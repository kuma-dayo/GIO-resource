...
suites = {
	...
	},
	{
		...
		regions = { },
		triggers = { "GADGET_STATE_CHANGE_377007", "GADGET_STATE_CHANGE_377008" },
		rand_weight = 100
	}
}
...
function condition_EVENT_GADGET_STATE_CHANGE_377008(context, evt)
	if 377003 ~= evt.param2 or GadgetState.GearStart ~= evt.param1 then
		return false
	end
...
function action_EVENT_GADGET_STATE_CHANGE_377008(context, evt)
	...
	-- 将configid为 377004 的物件更改为状态 GadgetState.GearStart
	if 0 ~= ScriptLib.SetGadgetStateByConfigId(context, 377004, GadgetState.GearStart) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_configId")
			return -1
		end 
