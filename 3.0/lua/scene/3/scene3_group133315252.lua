...
suites = {
	{
		...
		regions = { },
		triggers = { "GADGET_STATE_CHANGE_252004", "GADGET_STATE_CHANGE_252005", "GROUP_LOAD_252006", "GROUP_LOAD_252007" },
		rand_weight = 100
	}
}
...
-- 触发条件
function condition_EVENT_GROUP_LOAD_252006(context, evt)
	if GadgetState.GatherDrop ~= ScriptLib.GetGadgetStateByConfigId(context, 133315252, 252002) then
		return false
	end
...
-- 触发操作
function action_EVENT_GROUP_LOAD_252006(context, evt)
	-- 创建id为252003的gadget
	if 0 ~= ScriptLib.CreateGadget(context, { config_id = 252003 }) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : create_gadget")
	  return -1
