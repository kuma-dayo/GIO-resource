...
function Initialize()
	...
	--挑战计数进度-总数
	table.insert(variables,{ name = "score_total", value = 0})
	--挑战计数进度-连续上钩
	table.insert(variables,{ name = "score_unmiss", value = 0})
	--挑战计数进度-最大Bonus时长
	table.insert(variables,{ name = "bonus_time", value = 0})

	--隐藏任务用标志 0-未满足任务条件 1-已满足条件，可触发任务
		--这个标志不做no_refresh，因为任务那边希望每次都触发
	table.insert(variables,{ name = "quest_mark", value = 0})
