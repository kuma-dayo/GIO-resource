...
-- 触发器
triggers = {
	{ config_id = 1270002, name = "VARIABLE_CHANGE_270002", event = EventType.EVENT_VARIABLE_CHANGE, source = "", condition = "condition_EVENT_VARIABLE_CHANGE_270002", action = "action_EVENT_VARIABLE_CHANGE_270002", trigger_count = 0 },
...
-- 变量
variables = {
	{ name = "fishing_count", value = 0, no_refresh = true }
}
...
suites = {
	...
	},
	{
		...
		gadgets = { 270001 },
		regions = { },
		triggers = { "VARIABLE_CHANGE_270002", "FISHING_QTE_FINISH_270003" },
		rand_weight = 100
	}
}
...
-- 触发条件
function condition_EVENT_VARIABLE_CHANGE_270002(context, evt)
...
-- 触发操作
function action_EVENT_VARIABLE_CHANGE_270002(context, evt)
...
function action_EVENT_FISHING_QTE_FINISH_270003(context, evt)
	-- 针对当前group内变量名为 "fishing_count" 的变量，进行修改，变化值为 1
	if 0 ~= ScriptLib.ChangeGroupVariableValue(context, "fishing_count", 1) then
