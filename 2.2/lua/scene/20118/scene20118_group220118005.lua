...
function action_EVENT_TIMER_EVENT_5008(context, evt)
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 220118005, 5007, {1}) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_wok_options_by_configid")
		return -1
	end
	
	-- 改变指定group组220118005中， configid为5003的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 220118005, 5003, GadgetState.GearStart) then
...
function action_EVENT_GADGET_STATE_CHANGE_5009(context, evt)
	-- 改变指定group组220118005中， configid为5004的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 220118005, 5004, GadgetState.Action01) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
...
function action_EVENT_GADGET_STATE_CHANGE_5010(context, evt)
	-- 改变指定group组220118005中， configid为5006的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 220118005, 5006, GadgetState.Action01) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
...
function action_EVENT_GADGET_STATE_CHANGE_5011(context, evt)
	-- 改变指定group组220118005中， configid为5005的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 220118005, 5005, GadgetState.Action01) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
...
function action_EVENT_SELECT_OPTION_5012(context, evt)
	-- 改变指定group组220118005中， configid为5001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 220118005, 5001, GadgetState.GearAction2) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
	-- 改变指定group组220118005中， configid为5003的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 220118005, 5003, GadgetState.GearStop) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
	-- 改变指定group组220118005中， configid为5006的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 220118005, 5006, GadgetState.GearStop) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
	-- 改变指定group组220118005中， configid为5005的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 220118005, 5005, GadgetState.GearStop) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
	-- 改变指定group组220118005中， configid为5004的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 220118005, 5004, GadgetState.GearStop) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
	-- 删除指定group： 220118005 ；指定config：5007；物件身上指定option：1；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 220118005, 5007, 1) then
