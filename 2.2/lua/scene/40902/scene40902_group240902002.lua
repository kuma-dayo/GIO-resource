...
function condition_EVENT_ANY_MONSTER_DIE_2004(context, evt)
	--判断当前场景死亡的怪物总数不少于3
	if ScriptLib.GetMonsterDieCount(context) ~= 3 then
		return false
	end
...
function condition_EVENT_ANY_MONSTER_DIE_2005(context, evt)
	--判断当前场景死亡的怪物总数不少于4
	if ScriptLib.GetMonsterDieCount(context) ~= 4 then
		return false
	end
...
function condition_EVENT_ANY_MONSTER_DIE_2012(context, evt)
	--判断当前场景死亡的怪物总数不少于1
	if ScriptLib.GetMonsterDieCount(context) ~= 1 then
		return false
	end
...
function condition_EVENT_ANY_MONSTER_DIE_2026(context, evt)
	--判断当前场景死亡的怪物总数不少于2
	if ScriptLib.GetMonsterDieCount(context) ~= 2 then
		return false
	end
