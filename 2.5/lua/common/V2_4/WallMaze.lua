...
function Initialize()
    ...
    for i = 1,#StartWallMap do
        table.insert(variables,{ name = "wall_"..i, value = 0})
    end
    for i = 1,#StartBlockerMap do
        table.insert(variables,{ name = "blocker_"..i, value = 0})
    end
    
    if (SpecialBlockerMap ~= nil) then
        for i = 1,#SpecialBlockerMap do
            for j = 1,#SpecialBlockerMap[i] do
                table.insert(variables,{ name = "special_blocker_"..i.."_"..j, value = 0})
            end
        end
    end
    table.insert(variables,{ name = "is_moving", value = 0, no_refresh = true})
    
    table.insert(variables,{ name = "has_succeeded", value = 0, no_refresh = true})
end
