...
function LF_Initialize_Level()
	...
	local var = { name = "lastFloorIndex", value = 0, no_refresh = false } --上一次踩亮的地板顺位Index ，每次开始游戏重置
	variables[var.name] = var
	var = { name = "lastConfigID", value = 0, no_refresh = false } --上一次踩亮的地板configID ，每次开始游戏重置
	variables[var.name] = var
	var = { name = "totalFloorNum", value = defs.FloorNum, no_refresh = false } --地板计数器，侦测玩法结束变量
	variables[var.name] = var
	var = { name = "FloorPlayStart", value = 0, no_refresh = false } --玩法开始变量，设为1时玩法开启 ， 玩法失败时会置为0
	variables[var.name] = var
	var = { name = "FloorPlayEnd", value = 0, no_refresh = true } --玩法结束变量，设为1时玩法结束，需要设计师侦测
	variables[var.name] = var
